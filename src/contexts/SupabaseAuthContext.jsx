...
export const AuthProvider = ({ children }) => {
  ...
  // ✅ (novo) estado da sessão única (não quebra nada, só adiciona)
  ...
  const checkPremiumStatus = useCallback(async (userId) => {
    ...
    try {
      ...
      // ✅ (novo) se virou premium, para o polling automático (pra não ficar batendo atoa)
      if (hasActiveSub) {
        stopPremiumPolling();
      }
    } catch (err) {
    ...
    async (userId) => {
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (!ok) {
          ...
          // ✅ só derruba se falhar 2x seguidas (evita lag/falso positivo)
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      try {
        ...
        if (currentUser) {
          ...
          // ✅ grace period antes de começar a derrubar
          setTimeout(() => {
    ...
    const {
      ...
      if (currentUser) {
        ...
        // ✅ grace period antes de começar o polling (evita voltar pro login)
        setTimeout(() => {
          startSessionPolling(currentUser.id);
        }, GRACE_AFTER_LOGIN_MS);
      } else {
  ...
  // ✅ quando usuário volta pra aba/janela, sincroniza premium (muito útil pro Pix)
  useEffect(() => {
    const onFocus = async () => {
