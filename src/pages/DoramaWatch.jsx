...
export default function DoramaWatch() {
  ...
  // Para desativar em emergência, trocar para false.
  const playerFixOn = true;
  ...
  // ✅ O rótulo do alternativo é sempre o oposto do idioma principal do
  // dorama — funciona nos dois sentidos (dublado+legendado alt OU
  // legendado+dublado alt).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // (mais curto ainda pra tesagencia); aqui só é o piso mínimo.
      const interval = Math.max(2, data.heartbeat_interval_seconds ?? 6) * 1000;
