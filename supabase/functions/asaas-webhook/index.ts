...
// infinitepay-reconcile e whatsapp-sales-bot - mantenha todas em sincronia se
// o criterio de elegibilidade mudar.
async function creditReferralIfEligible(supabase: any, referredId: string): Promise<ReferralCreditResult> {
  try {
...
async function resolvePendingReferralsForReferrer(supabase: any, referrerId: string): Promise<PendingResolveResult> {
  try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: subErr } = await supabase.from("subscriptions").upsert({
      user_id: userId, status: "active", start_at: now.toISOString(), end_at: endAt.toISOString(),
      current_period_end: endAt.toISOString(), plan_name: planName, plan_interval: planInterval,
      source: "asaas", provider: "asaas", provider_ref: asaasPaymentId, order_nsu: externalReference,
      price_id: plan === "quarterly" ? "asaas_pix_4790" : "asaas_pix_1690",
      is_manual: false, notes: `PIX Asaas - ${planName}`, last_renewed_at: now.toISOString(),
    }, { onConflict: "user_id" });

    if (subErr) { console.error("[asaas-webhook] subscription error:", subErr); return json({ ok: false, error: "erro ao liberar assinatura" }, 500); }

    await supabase.from("profiles").update({ active: true, subscription_active_until: endAt.toISOString(), updated_at: now.toISOString() }).eq("id", userId);
