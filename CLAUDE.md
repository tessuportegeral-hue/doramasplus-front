...
- **A trava de verdade é na hora de ASSISTIR**, via edge function `claim-playback` (usa `playback_sessions` + `subscriptions.max_concurrent_streams`, hoje 1 por conta) — `DoramaWatch.jsx`, efeito "CLAIM-PLAYBACK".
- Quando bate no limite, aparece o botão "Limite de Reprodução? Clique aqui para resolver". **Desde 08/08/2026 ele manda pra `/login`** (não força mais entrada/derruba o outro dispositivo) — o force-claim (`force:true`) não estava resolvendo de verdade na prática, só logar de novo resolvia. Código do force-claim removido de `DoramaWatch.jsx`; a Dora também não oferece mais esse botão, só orienta `/login` direto.
...
- Heartbeat: 3s pra todo mundo (liberado geral 29/07, era só tesagencia no teste inicial) — limpeza de sessão zumbi em 9s.
